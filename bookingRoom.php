<?php // Script 13.9 - edit_info.php
 ...
 if (isset($_GET['roomNo']) && is_numeric($_GET['roomNo']) && ($_GET['roomNo'] > 0) ) { // Display the entry in a form:
     // Define the query.
     $query = "SELECT roomNo, capacity, start_date, end_date, start_time, end_time, reserved FROM roomReservation WHERE roomNo={$_GET['roomNo']}";
 if ($result = mysqli_query($dbc, $query)) { // Run the query.
   ...
   // Reserved     
      print '<p class="col-sm-12"><label class="col-sm-3">Reserved</label>
               <input class="btn btn-light" type="text" name="reserved" value="' .htmlentities($row['reserved']) . '"></p>';
   // Start Date   
   ...
   // End Time	       
     ...
     $reserved= mysqli_real_escape_string($dbc, trim(strip_tags($_POST['reserved'])));
     
     if (!$problem) { // Update the query if the room is available.
       if($reserved === "reserved") {
     	print '<p class="error-msg">You are unable to book this room because it is already been reserved.</p>';
       } else {       
       ...
       if ($result = mysqli_query($dbc, $query)) {
         print '<p class="error-msg">The Room is reserved for you.</p>';
        } else {
...
?>
   ...
   // JQuery to disable the past days in the Calender
